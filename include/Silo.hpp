...
struct Silo {
	...
	constexpr static DBoptlist const *optlist = NULL;
	...
	void writeCoordinates(auto const &origin, Real dx, Integer gridWidth) {
		...
		DBPutQuadmesh(db_, meshname, coordnames, coords_.data(), dims_.data(), D, datatype, coordtype, optlist);
	}
	void writeData(auto begin, auto const &names) {
		...
		for (Integer f = 0; f < nfields; f++) {
			...
			DBPutPointvar1(db_, names[f].c_str(), meshname, varptr, nels_, datatype, optlist);
		}
	}
	~Silo() {
		DBClose(db_);
